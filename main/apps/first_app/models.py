...
class userManager(models.Manager):
    ...
    def job_validation(self, postData):
        ...
        if len(postData['desc']) < 10:
            errors['title'] = 'Description must contain at least 10 characters'
        if len(postData['location']) < 1:
            errors['location'] = 'Location input cannot be empty'
        if len(postData['location']) < 3:
            errors['location'] = 'Location input must contain at least 3 characters'
        return errors
